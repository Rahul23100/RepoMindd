...
# Features: AI Analysis, Code Stats, Language Detection, Contributors
# ===============================================
...
def generate_ai_summary(analysis: Dict, api_key: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    prompt = f"""
    You are RepoMind AI, an expert software repository analyst. Analyze this GitHub repository comprehensively.
    ...
    2. **⚡ Key Features & Capabilities**
       - List the main features (bullet points)
    ...
    4. **📈 Development Activity & Health**
       - Development pace and momentum
       - Focus areas from recent commits
    ...
    6. **🚀 Future Direction**
       - Potential roadmap based on recent activity
       - Areas of active development
    
    7. **💡 Recommendations**
       - For potential contributors
       - For users considering this project
    
    8. **⚠️ Potential Concerns**
       - Any red flags or areas needing attention
       - Technical debt indicators
    
    Format your response in clear markdown with emojis for visual appeal. Be specific and actionable.
    """
...
def generate_code_quality_report(analysis: Dict, api_key: str) -> Tuple[Optional[str], Optional[str]]:
    """
    Generate a code quality and best practices report.
    """
    genai.configure(api_key=api_key)
    
    prompt = f"""
    As a senior software architect, analyze the code quality indicators for this repository:
    ...
    1. **Code Organization** - File structure and modularity
    2. **Commit Quality** - Message clarity and commit size
    3. **Development Practices** - Based on patterns observed
    4. **Recommendations** - Top 3 specific improvements
    ...
    try:
        ...
        return None, f"❌ Code quality analysis error: {str(e)}"
...
def main():
    ...
    # Initialize a single session state variable for the URL
    ...
    with col1:
        ...
        # We assign it a new key to avoid conflicts.
        st.text_input(
            "GitHub Repository URL",
            placeholder="https://github.com/owner/repository",
            help="Enter a public GitHub repository URL",
            key="url_input_widget", # <-- Use a different key for the widget itself
            value=st.session_state.repo_url, # <-- Set the displayed value from our master state
            on_change=lambda: setattr(st.session_state, 'repo_url', st.session_state.url_input_widget) # <-- Update the master state when user types
        )
    ...
    # Example repositories now update the single 'repo_url' state variable
    with st.expander("💡 **Try Example Repositories**"):
        col1, col2, col3 = st.columns(3)
        if col1.button("Streamlit", use_container_width=True):
            st.session_state.repo_url = "https://github.com/streamlit/streamlit"
            st.rerun()
        if col2.button("FastAPI", use_container_width=True):
            st.session_state.repo_url = "https://github.com/tiangolo/fastapi"
            st.rerun()
        if col3.button("TensorFlow", use_container_width=True):
            st.session_state.repo_url = "https://github.com/tensorflow/tensorflow"
            st.rerun()

    # The analyze button now reads from the single source of truth
    if analyze_button:
    ...
    if st.session_state.analysis:
        ...
        tab1, tab2, tab3, tab4 = st.tabs(["🤖 AI Summary", "📈 Code & Commits", "🧐 Code Quality", "📖 README"])
